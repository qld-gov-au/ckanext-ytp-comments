  ...
  return {
    ...
    _onClick: function (event) {
      ...
      jQuery.get('/comments/' + (thread_id ? thread_id : comment_id) + '/' + action, function() {
        ...
        jQuery(element.el).parent().find('.comments-' + inverse + (thread_id ? '' : '-thread')).removeClass('hidden').removeClass('d-none');
        if (!comment_id) {
          jQuery('.comments-follow-thread').addClass('hidden d-none');
          jQuery('.comments-mute-thread').removeClass('hidden').removeClass('d-none');
        }
      })
      .fail(function() {
